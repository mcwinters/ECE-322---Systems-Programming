...
void* mm_malloc (size_t size) {
  ...
  ptrFreeBlock = searchFreeList(reqSize);       // Search for a free block

  if (ptrFreeBlock == NULL){
    requestMoreSpace(reqSize);                  // If there is not enough room, get more heap space
    ptrFreeBlock = searchFreeList(reqSize);     // Search for free block again
  }

  blockSize = SIZE(ptrFreeBlock->sizeAndTags);  // Obtain block size

  precedingBlockUseTag = ptrFreeBlock->sizeAndTags
                          & TAG_PRECEDING_USED; // Store the preceding block's used tag

  removeFreeBlock(ptrFreeBlock);                // Remove free block

  size_t MIN_BLOCK_DIFFERENCE = blockSize - reqSize;

  /* Split the block if MIN_BLOCK_SIZE > (blockSize - reqSize) */
  if (MIN_BLOCK_SIZE > MIN_BLOCK_DIFFERENCE) {

    /* Obtain the pointer to the next block */
    BlockInfo* NextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);

    NextBlock->sizeAndTags |= TAG_PRECEDING_USED;  // Set the next block's previous used tag
    ptrFreeBlock->sizeAndTags |= TAG_USED;         // Set the used tag of the block
  } else {
    ptrFreeBlock->sizeAndTags = precedingBlockUseTag | reqSize; // Set the block's size and tags
    ptrFreeBlock->sizeAndTags |= TAG_USED;                      // Set the used tag of the block

    /* Set preceding used tag of free block */
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize)) = TAG_PRECEDING_USED | MIN_BLOCK_DIFFERENCE;

    /* Set the preceding used tag within boundary tag */
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = TAG_PRECEDING_USED | MIN_BLOCK_DIFFERENCE;

    /* Insert free block to the free list */
...
void mm_free (void *ptr) {
  ...
  /* Obtain block's pointer */
  blockInfo = (BlockInfo*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  /* Obtain block size */
  payloadSize = SIZE(blockInfo->sizeAndTags);

  /* Obtain pointer to the next block */
  NextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, payloadSize);

  /* Set the next block's preceding used tag to 0 */
  NextBlock->sizeAndTags &= (~TAG_PRECEDING_USED);

  blockInfo->sizeAndTags &= (~TAG_USED);  // Set the block's used tag to 0
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  /* If ptr = NULL, return malloc */
  if (ptr == NULL) return mm_malloc(size);

  /* If size = 0 then call mm_free, return 0 */
  if (size == 0) {
      mm_free(ptr);
      return 0;
  }
  ...
  // Custom defination of memcpy
  char *cptr = (char *)ptr;
  char *cnewptr = (char *)newptr;

  for(int index = 0; index < oldsize; index++){
        cnewptr[index] = cptr[index];
    }
